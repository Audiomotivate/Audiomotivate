...
export default function AllGuides() {
  ...
  const filteredGuides = guides?.filter(product =>
    ...
    return (
      <>
        ...
        <div className="container mx-auto px-4">
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-montserrat font-bold text-center mb-6">Todas las Guías</h1>
      ...
      <main className="min-h-screen bg-white pt-24 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="md:hidden space-y-4 mb-6">
            ...
            <h1 className="text-xl font-montserrat font-bold text-center">Todas las Guías PDF</h1>
          </div>
          ...
          <div className="hidden md:flex items-center justify-between mb-6 gap-4">
            ...
            <div className="flex items-center justify-center flex-1 px-4">
              ...
              <h1 className="text-2xl md:text-3xl font-montserrat font-bold text-center">Todas las Guías PDF</h1>
            </div>
          ...
          <p className="text-lg text-gray-700 max-w-3xl mx-auto text-center">
            Ejercicios prácticos y material de apoyo para aplicar lo aprendido en tu día a día.
          </p>
        </div>
        ...
        <div className="mb-16">
          ...
          <div className="relative">
            <button 
              ...
              onClick={() => {
                const container = document.getElementById('bestsellers-guides-carousel');
            ...
            <div 
              id="bestsellers-guides-carousel"
              className="flex gap-4 overflow-x-auto scrollbar-hide pb-4 px-8"
              style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
            >
              {(searchTerm ? filteredGuides : guides?.filter(p => p.isBestseller))?.slice(0, 6).map((product) => (
        ...
        <div className="mb-16">
          ...
          <div className="relative">
            <button 
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 transition-all duration-300"
              onClick={() => {
                const container = document.getElementById('new-releases-guides-carousel');
                if (container) container.scrollBy({ left: -300, behavior: 'smooth' });
              }}
            >
              <ChevronLeft className="h-5 w-5" />
            ...
            <div 
              id="new-releases-guides-carousel"
              className="flex gap-4 overflow-x-auto scrollbar-hide pb-4 px-8"
              style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
            >
              {(searchTerm ? filteredGuides : guides?.filter(p => p.isNew))?.slice(0, 6).map((product) => (
                <Link href={`/product/${product.id}`} key={product.id} className="flex-shrink-0">
                  <div className="group cursor-pointer w-36 md:w-48 transition-all duration-300 hover:scale-105">
                    <div className="relative mb-3">
                      <img 
