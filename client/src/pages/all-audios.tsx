...
import { Clock, ChevronLeft, ChevronRight, Star, Search, Volume2, ArrowLeft } from "lucide-react";
...
export default function AllAudios() {
  ...
  const { data: audios, isLoading } = useQuery<Product[]>({
    queryKey: ['/api/products/type/video'],
  });

  // Filter products based on search term
  const filteredAudios = audios?.filter(product =>
  ...
  if (isLoading) {
    return (
      <>
        ...
        <main className="min-h-screen bg-white pt-24 py-8">
        <div className="container mx-auto px-4">
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-montserrat font-bold text-center mb-6">Todos los Videos</h1>
      ...
      <main className="min-h-screen bg-white pt-24 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="md:hidden space-y-4 mb-6">
            ...
            <h1 className="text-xl font-montserrat font-bold text-center">Todos los Audios de Superación</h1>
          </div>
          ...
          <div className="hidden md:flex items-center justify-between mb-6 gap-4">
            ...
            <h1 className="text-2xl md:text-3xl font-montserrat font-bold text-center flex-1 px-4">Todos los Audios de Superación</h1>
          ...
          <p className="text-lg text-gray-700 max-w-3xl mx-auto text-center">
            Contenido inspirador para superar obstáculos y mantener tu motivación al máximo nivel.
          </p>
        </div>
        ...
        <div className="mb-16">
          ...
          <div className="relative">
            <button 
              ...
              onClick={() => {
                const container = document.getElementById('bestsellers-videos-carousel');
            ...
            <div 
              id="bestsellers-videos-carousel"
              className="flex gap-4 overflow-x-auto scrollbar-hide pb-4 px-8"
              style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
            >
              {(searchTerm ? filteredAudios : audios?.filter(p => p.isBestseller))?.slice(0, 6).map((product) => (
                <Link href={`/product/${product.id}`} key={product.id} className="flex-shrink-0">
                  <div className="group cursor-pointer w-36 md:w-48 transition-all duration-300 hover:scale-105">
                    ...
                    <div className="px-1">
                      <h3 className="font-bold text-gray-800 mb-1 line-clamp-2 text-sm md:text-base group-hover:text-accent transition-colors leading-tight">
                        {product.title}
                      </h3>
        ...
        <div className="mb-16">
          ...
          <div className="relative">
            <button 
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 transition-all duration-300"
              onClick={() => {
                const container = document.getElementById('new-releases-videos-carousel');
                if (container) container.scrollBy({ left: -300, behavior: 'smooth' });
              }}
            >
              <ChevronLeft className="h-5 w-5" />
            ...
            <div 
              ...
              style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
            >
              {(searchTerm ? filteredAudios : audios?.filter(p => p.isNew))?.slice(0, 6).map((product) => (
                <Link href={`/product/${product.id}`} key={product.id} className="flex-shrink-0">
                  <div className="group cursor-pointer w-36 md:w-48 transition-all duration-300 hover:scale-105">
                    <div className="relative mb-3">
                      <img 
